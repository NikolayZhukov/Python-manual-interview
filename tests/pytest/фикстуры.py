...
@pytest.fixture
def sample_data():
    print("\n1) Выполняется фикстура перед тестом")  # выполняется перед тестом
    data = [1, 2, 3]
    yield data  # вместо return используем yield
    print("\n3) Выполняется фикстура после теста")  # выполняется после теста
    # Здесь можно добавить любые пост-обработки, кроме подключения к БД
    # Например: очистка временных файлов, сброс состояния и т.д.

# тест с использованием фикстуры
def test_sum(sample_data): # фикстура передается в качестве аргумента
